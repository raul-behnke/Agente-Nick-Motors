...
from zoi_agent.agent import guards as guards_mod
...
async def _run_turn_inner(
    ...
    # GUARDS determinísticos Nick (camada Veltron). Usa `state` PRÉ-merge p/
    # contar identidade-IA com o delta deste turno sem duplicar.
    guard_outcome = guards_mod.apply_guards(
        state, update, last_message, ai_identity_threshold=settings.ai_identity_admit_at
    )
    ...
    try:
        team_out = await run_team_turn(
            ...
            ai_identity_text=guard_outcome.ai_identity_text,
        )
    except Exception as e:
    ...
    # ESCALA SILENCIOSA (Nick): opt-out / irritação (score≤3) escalam SEM bolha
    # de despedida ao cliente — mandar mensagem aqui só irrita mais. Demais
    # handoffs (qualificado, fora-escopo) comunicam normalmente via persona.
    silent_handoff = update.terminal_reason == "handoff_solicitado" and (
        update.intent == "opt_out" or new_state.last_sentiment == "irritado"
    )

    # Send sob shield: imune a preempção
    if silent_handoff:
        log.info("silent_handoff_no_bubbles", contact_id=contact_id, intent=update.intent)
    else:
