...
from zoi_agent.team.nick import NICK_INSTRUCTIONS, consultar_faq
...
def build_nick_agent() -> Agent:
    """NICK como Agent standalone. Output: BubbleSequence (sem inventory_decision)."""
    return Agent(
        name="Nick",
        model=OpenAIChat(
        ...
        description=(
            "Pré-atendimento da Nick Motors Seminovos (São Paulo/SP). Consultivo, "
            "objetivo, conduz qualificação de leads via WhatsApp."
        ),
...
async def run_team_turn(
    *,
    ...
    nick_agent = build_nick_agent()
    nick_input = json.dumps(patricia_payload, ensure_ascii=False, default=str)
    try:
        _t0 = time.perf_counter()
        result = await asyncio.wait_for(
            nick_agent.arun(input=nick_input), timeout=_ARUN_TIMEOUT_S
        )
    except TimeoutError:
        log.error("nick_arun_timeout", timeout_s=_ARUN_TIMEOUT_S)
        raise RuntimeError(f"Nick.arun timeout >{_ARUN_TIMEOUT_S}s") from None
    _record_agno_usage(
        result,
        component="nick",
    ...
    if not isinstance(content, BubbleSequence):
        log.error("nick_output_not_schema", type=type(content).__name__)
    ...
    # 2º passe: EDITOR de estilo (camada Veltron). Humaniza, fronteira-de-fato.
    # Degrada gracioso pro rascunho se falhar (nunca trava o turno).
    try:
        hist_txt = "\n".join(
        ...
        seq = await editor.run_editor(
            ...
            historico_recente=hist_txt,
            modelo_em_foco=vehicle_in_focus,
            has_cards=bool(cards_bolhas),
            faq_yaml=faq_yaml or "",
            sentiment=update.sentiment,
