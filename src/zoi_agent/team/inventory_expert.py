"""EstoqueExpert — Agno Agent especialista no estoque da Nick Motors.

Recebe contexto da conversa, raciocina sobre os veículos do pátio (no system
prompt), e devolve InventoryDecision estruturada. O NICK (Team leader) consome
...
async def puxar_ficha_veiculo(external_id: str) -> dict[str, Any]:
    ...
    não traz a info, devolva indicação clara — NÃO invente. O NICK vai
...
_INSTRUCTIONS = [
    "Você é o EstoqueExpert da Nick Motors — especialista no pátio.",
    ...
    "estruturada pro NICK (Team leader) tecer as bolhas finais.",
    "",
    ...
    "Hatch é uma categoria comum no estoque.",
    "",
    ...
    "Use pra passar pro NICK o ÂNGULO de venda baseado na história do "
    ...
    "O NICK pode reescrever. Em dúvida, deixe None.",
    "",
...
async def build_inventory_expert() -> Agent:
    ...
    inventory_context = (
        f"INVENTÁRIO ATUAL DA NICK MOTORS ({len(inv)} veículos ativos):\n"
    ...
    return Agent(
        ...
        description="Especialista no estoque da Nick Motors (São Paulo/SP).",
