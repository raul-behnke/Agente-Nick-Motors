...
class InventoryDecision(BaseModel):
    ...
    NICK (leader) recebe essa decisão e tece bolhas finais. Orchestrator
    ...
    pergunta_refinamento: str | None = Field(
        ...
        description=(
            ...
            "NICK veste persona em cima. Ex: 'Câmbio automático ou manual?'"
        ),
    )
    hint_narrativo: str | None = Field(
        default=None,
        description=(
            "Ângulo de venda / tom pro NICK incorporar. "
    ...
    texto_sugerido_apresentacao: str | None = Field(
        default=None,
        description=(
            "Rascunho de bolha narrativa pro NICK vestir persona. Opcional. "
            "NICK pode reescrever, mas mantém o sentido."
        ),
    )
...
class BubbleSequence(BaseModel):
    ...
    Quando você (NICK) delegou ao EstoqueExpert neste turno, COPIE a
