...
def _normalize_vehicle(v: dict) -> dict:
    ...
    # Nick stock flip-flopa entre 2 schemas: enxuto (km/url) e completo
    # (quilometragem/url_anuncio/imagens). Faz a ponte pros 2.
    if "quilometragem" not in out and v.get("km") is not None:
        out["quilometragem"] = v.get("km")
    if "url_anuncio" not in out and v.get("url"):
        out["url_anuncio"] = v.get("url")
    if "opcionais" not in out:
        out["opcionais"] = v.get("acessorios") or v.get("diferenciais") or []
    if "carroceria" not in out and (v.get("categoria") or v.get("tipo_veiculo")):
        out["carroceria"] = v.get("categoria") or v.get("tipo_veiculo")
    if "descricao" not in out:
        out["descricao"] = v.get("descricao_resumida") or v.get("texto_busca_ia") or v.get("searchText") or ""
